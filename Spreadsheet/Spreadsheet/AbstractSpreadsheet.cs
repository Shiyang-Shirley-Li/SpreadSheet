...
/// 
/// I, Shiyang(Shirley) Li, certify that Professor Joe Zachary wrote this code and I add some comments for the "Visit" mehtod 
/// and did not copy it in part or whole from another source.  All references used in the completion of the assignment are 
...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            visited.Add(name);//mark the name as visted
            foreach (String n in GetDirectDependents(name))//get the direct dependents of name
            {
                if (n.Equals(start))//if the direct dependent of name is the same as start name, then it is a circular dependency
                {
                    throw new CircularException();
                }
                else if (!visited.Contains(n))//if we did not vist the direct dependent, then do the recursion
                {
                    Visit(start, n, visited, changed);
                }
            }
            changed.AddFirst(name);//add the dependents and the name itself in correct order
        }
